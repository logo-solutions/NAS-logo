...
echo "=== IMMICH IMPORT (--skip-hash): $DIRNAME ==="
echo "Source: $SOURCE"
...
bash bin/immich-backup-db-only.sh > /dev/null 2>&1 && echo "    ✓ Backup OK" || echo "    (backup skipped)"

# ── 2. Import direct via immich-go (--skip-hash pour doublons) ──────────────────
echo "[2] Import (--skip-hash pour détecter doublons)..."
...
echo "  Added:  $ADDED"
echo "  Skipped (duplicates): $(grep -c "skipped\|duplicate" "$LOG" 2>/dev/null || echo 0)"

echo ""
echo "Restarting Immich..."
cd "$IMMICH_HOME"
docker compose restart 2>&1 | grep -E 'immich|done|error' || true
